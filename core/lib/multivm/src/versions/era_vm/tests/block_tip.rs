...
use crate::{
    era_vm::{
        tests::tester::{default_l1_batch, get_empty_storage, VmTesterBuilder},
        tracers::{dispatcher::TracerDispatcher, pubdata_tracer::PubdataTracer},
    },
...
fn execute_test(test_data: L1MessengerTestData) -> TestStatistics {
    ...
    for code in &test_data.bytecodes {
        vm.storage
            .borrow_mut()
            .store_factory_dep(hash_bytecode(code), code.clone());
    }
    ...
    let ergs_before = vm.vm.inner.execution.gas_left().unwrap();
    ...
    let ergs_after = vm.vm.inner.execution.gas_left().unwrap();
