...
export async function deployL2(args: any[] = [], includePaymaster?: boolean, includeWETH?: boolean, nativeToken?: boolean) {
    await utils.confirmAction();
    ...
    await utils.spawn(`${baseCommandL1} initialize-bridges ${nativeToken ? ' --native-erc20' : ''} ${args.join(' ')} | tee deployL2.log`);

    if (includePaymaster) { 
        await utils.spawn(`${baseCommandL2} deploy-testnet-paymaster ${nativeToken ? ' --native-erc20' : ''} ${args.join(' ')} | tee -a deployL2.log`);
    }
    ...
    if (includeWETH) {
        await utils.spawn(`${baseCommandL1} initialize-weth-bridges ${nativeToken ? ' --native-erc20' : ''} ${args.join(' ')} | tee -a deployL1.log`);
    }
